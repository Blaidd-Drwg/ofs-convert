...
void set_size(uint32_t inode_no) {
    ext4_inode& inode = get_existing_inode(inode_no);
    uint64_t blocks = from_lo_hi(inode.i_blocks_lo, inode.l_i_blocks_high);
    set_lo_hi(inode.i_size_lo, inode.i_size_high, blocks * 512);
}
