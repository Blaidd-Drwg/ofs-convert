...
void append_to_new_idx_path(uint16_t depth, extent *ext, ext4_extent_idx *idx, ext4_super_block *sb) {
    for (int i = depth; i > depth; i--) {
        ...
        *idx = {
        ...
        ext4_extent_header *header = (ext4_extent_header *) block_start(idx->ei_leaf_lo, *sb);
        header->eh_entries = 1;
        header->eh_max = block_size(*sb) / sizeof(ext4_extent);
        header->eh_depth = i;
...
bool append_to_extent_tree(extent *ext, ext4_extent_header *root_header, ext4_super_block *sb) {
    ...
    ext4_extent_header *child_header = (ext4_extent_header *) block_start(child_block, *sb);
    if (append_to_extent_tree(ext, child_header, sb)) {
        return true;
    }

    if (entry_count < root_header->eh_max) {
        ext4_extent_idx *idx = (ext4_extent_idx *) (root_header + entry_count);
        append_to_new_idx_path(root_header->eh_depth - 1, ext, idx, sb);
        return true;
    } else {
        return false;
    }
...
void make_tree_deeper(ext4_extent_header *root_header, ext4_super_block *sb) {
    uint32_t block_no  = allocate_extent(1).physical_start;
    uint8_t *child_block = block_start(block_no, *sb);
    ...
    child_header->eh_max = block_size(*sb) / sizeof(ext4_extent);
...
void add_extent(extent *ext, uint32_t inode_number, ext4_super_block *sb) {
    ext4_inode *inode = get_inode(inode_number);
    ext4_extent_header *header = &(inode->ext_header);
    bool success = append_to_extent_tree(ext, header, sb);

    if (!success) {
        make_tree_deeper(header, sb);
        // attempt adding extent again, should succeed this time
        append_to_extent_tree(ext, header, sb);
    }
    inode->i_blocks_lo += ext->length;
}

void set_extents(uint32_t inode_number, fat_dentry *dentry, StreamArchiver *read_stream, ext4_super_block *sb) {
    ext4_inode *inode = get_inode(inode_number);
    inode->i_size_lo = dentry->file_size;
    extent *current_extent = (extent *) iterateStreamArchiver(read_stream, false, sizeof *current_extent);
    while (current_extent != NULL) {
        add_extent(current_extent, inode_number, sb);
